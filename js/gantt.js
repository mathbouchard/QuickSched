                ...
                if(options.data != null)
                {
                    ...
                    $.each(options.data, function(key,val) {
                        if(ganttval(val.starttimel, options.currw, options.mult) < wmin) {
                            wmin = ganttval(val.durationu, options.currw, options.mult);
                        }
                    ...
                    var it = 0;
                    $.each(options.data, function(key,val) {
                        var gblockin = $( "<li></li>" ).css('width', maxwidth+110).css('background', '#fff').addClass("ganttblock").attr("objid", -1)
                            .css('top', 120+it*50).appendTo( ulist );
